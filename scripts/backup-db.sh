...
#
# Env:
#   DB_PATH              (default: server/storage/openafd.db)
...
DB_PATH="${DB_PATH:-$REPO_ROOT/server/storage/openafd.db}"
...
TARGET="$BACKUP_DIR/openafd-$STAMP.db"
...
find "$BACKUP_DIR" -name "openafd-*.db.gz" -type f -mtime "+$RETENTION_DAYS" -delete

COUNT="$(find "$BACKUP_DIR" -name 'openafd-*.db.gz' -type f | wc -l | tr -d ' ')"
