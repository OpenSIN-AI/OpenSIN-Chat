...
`tooling/scripts/enable-multi-user.cjs` is an explicit maintenance tool for enabling
...
```bash
cp apps/api/storage/opensin.db \
  "apps/api/storage/opensin.db.backup.$(date +%Y%m%d-%H%M%S)"
```
...
node tooling/scripts/enable-multi-user.cjs
```
