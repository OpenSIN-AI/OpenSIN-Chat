...
const REQUEST_TIMEOUT_MS = Number(process.env.DEPENDENCY_HEALTH_REQUEST_TIMEOUT_MS || 8000);
const MAX_DURATION_MS = Number(process.env.DEPENDENCY_HEALTH_MAX_DURATION_MS || 90000);
...
async function main() {
  ...
  const timedOut = rows.length < names.length;
  if (timedOut) {
    console.warn(
      `[dep-health] time budget reached after ${MAX_DURATION_MS}ms — checked ${rows.length}/${names.length} dependencies.`,
    );
  }

  if (!networkOk && rows.length > 0 && rows.every((r) => r.skipped)) {
  ...
  if (JSON_OUT) {
    console.log(
      JSON.stringify(
        { years: YEARS, checked: rows.length, total: names.length, timedOut, deprecated, stale },
        null,
        2,
      ),
    );
  ...
  console.log(
    timedOut
      ? `[dep-health] PARTIAL OK — checked ${rows.length}/${names.length}`
      : "[dep-health] OK",
  );
}
