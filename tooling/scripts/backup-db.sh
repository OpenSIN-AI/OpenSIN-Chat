...
#
# Usage:
#   ./tooling/scripts/backup-db.sh                  # backup with defaults
#   BACKUP_DIR=/mnt/nas/opensin ./tooling/scripts/backup-db.sh
#
# Cron (daily at 03:30):
#   30 3 * * * cd /path/to/OpenSIN-Chat && ./tooling/scripts/backup-db.sh >> apps/api/storage/backups/backup.log 2>&1
#
# Env:
#   DB_PATH              (default: apps/api/storage/opensin.db)
#   BACKUP_DIR           (default: apps/api/storage/backups)
...
DB_PATH="${DB_PATH:-$REPO_ROOT/apps/api/storage/opensin.db}"
BACKUP_DIR="${BACKUP_DIR:-$REPO_ROOT/apps/api/storage/backups}"
