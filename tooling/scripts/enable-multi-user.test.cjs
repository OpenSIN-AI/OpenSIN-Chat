...
test("fails closed before database access when credentials are missing", () => {
  assert.throws(
    () =>
      runMigration({
        env: {
          OPENSIN_CHAT_DB_PATH: "/definitely/missing/opensin.db",
        },
      }),
    /Missing credentials/,
  );
...
test("command exits non-zero without an explicit password", () => {
  const result = spawnSync(
    process.execPath,
    [path.join(__dirname, "enable-multi-user.cjs")],
    {
      encoding: "utf8",
      env: {
  ...
  assert.doesNotMatch(result.stdout, /password|hash/i);
});
