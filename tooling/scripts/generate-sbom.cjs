...
const OUT_DIR = path.join(ROOT, "tooling", "artifacts", "sbom");
...
const WORKSPACES = [
  ...
  { name: "worker", dir: path.join(ROOT, "apps", "worker") },
];
...
function check() {
  if (!fs.existsSync(OUT_DIR)) {
    console.error("[sbom] tooling/artifacts/sbom/ missing — run `node tooling/scripts/generate-sbom.cjs`");
    process.exit(1);
  }
