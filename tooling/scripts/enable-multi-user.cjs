...
function loadServerDependency(name) {
  const candidates = [
    path.resolve(repoRoot, "apps", "api", "node_modules", name),
    name,
  ];
  ...
  throw new Error(
    `Dependency "${name}" was not found. Run "yarn install --frozen-lockfile" ` +
      `from the repository root first. Original error: ${lastError?.message}`,
  );
}
...
function resolveDatabasePath(env = process.env) {
  const requestedPath =
    ...
    path.resolve(repoRoot, "apps", "api", "storage", "opensin.db");
