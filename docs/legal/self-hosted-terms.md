...
OpenSIN Chat allows users to connect to external services (e.g., OpenAI, Anthropic, Pinecone). 
...
_by default, OpenSIN Chat does **everything on-device first** - so you would have to manually configure and enable these integrations to be subject to third party terms._
...
* **Environment Security:** The user is responsible for securing the host environment, including network firewalls, SSL/TLS encryption, and access control for the OpenSIN Chat instance.
...
* **License:** The OpenSIN Chat core is provided under the **MIT License**.
