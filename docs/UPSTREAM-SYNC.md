...
4. **Konflikte werden in der Gruppe sichtbar, nicht im Master.** Rebrand-Touch-Points (Display-Namen, `AnythingLLM` → `OpenSIN Chat`, Logo-Dateinamen) erzeugen die meisten Konflikte. Diese sind erwartet und werden vom Branding-Linter (`tooling/scripts/check-branding.sh`) in CI abgefangen.
...
bash tooling/scripts/upstream-sync/generate-patches.sh
# → erstellt tooling/scripts/upstream-sync/patches/01-..06-*/*.patch
# → schreibt tooling/scripts/upstream-sync/patches/MANIFEST.md

# 3. Manifest reviewen
cat tooling/scripts/upstream-sync/patches/MANIFEST.md
...
gh pr create --title "chore: sync upstream $(date +%Y-%m-%d)" \
             --body "Auto-generated via tooling/scripts/upstream-sync/. See MANIFEST.md."
```
...
| 03 | `voice-features`      | Server-Side STT (OpenAI) und Kokoro TTS                               | 2       |
...
- `tooling/scripts/upstream-sync/generate-patches.sh` — `git format-patch` + Verteilung in 6 Gruppen
- `tooling/scripts/upstream-sync/apply-patches.sh`    — `git am --3way` in Gruppen-Reihenfolge
- `tooling/scripts/check-branding.sh`                 — CI-Wächter gegen `AnythingLLM`/`Mintplex`-Leak
- `.github/workflows/branding-lint.yml`       — Linter läuft auf jedem Push + PR

## Siehe auch

- `docs/branding.md`         — Marken-Richtlinien + Whitelist
- `docs/legal/third-party.md`      — Upstream-Credit + Lizenz-Hinweise
