...
export default function GroupModal({ open, group, onClose, onSave }: Props) {
  ...
  const inputClass =
    "h-10 w-full rounded-lg border border-theme-modal-border bg-theme-bg-secondary px-3 text-sm text-theme-text-primary outline-none focus:border-theme-text-secondary";
  const areaClass =
    "min-h-28 w-full resize-y rounded-lg border border-theme-modal-border bg-theme-bg-secondary px-3 py-2 text-sm text-theme-text-primary outline-none focus:border-theme-text-secondary";
  ...
  return (
    ...
    >
      <form
        onSubmit={submit}
        className="w-full max-w-2xl overflow-hidden rounded-2xl border border-theme-modal-border bg-theme-bg-primary shadow-2xl"
      >
        <header className="flex items-start gap-3 border-b border-theme-modal-border px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-base font-semibold text-theme-text-primary">
              {group ? "Gruppe bearbeiten" : "Organisation oder Gruppe anlegen"}
            </h2>
            <p className="mt-1 text-xs text-theme-text-secondary">
              Fasse mehrere Ansprechpartner, Mitarbeiter und Domains zu einem
        ...
        <div className="grid gap-4 px-5 py-5 sm:grid-cols-2">
          <label className="sm:col-span-2">
            ...
            <input
              ...
              placeholder="Hausverwaltung, Jobcenter, Kunde …"
        ...
        <footer className="flex justify-end gap-2 border-t border-theme-modal-border px-5 py-4">
          ...
          >
            {saving ? "Speichert …" : "Gruppe speichern"}
          </button>
        </footer>
      </form>
    </div>
