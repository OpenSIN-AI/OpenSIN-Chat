...
const VECTOR_DBS: VectorDB[] = [
  {
    ...
    description:
      "100% local vector DB that runs on the same instance as OpenSIN Chat.",
  },
  {
    name: "PGVector",
    value: "pgvector",
...
export default function GeneralVectorDatabase() {
  ...
  const [selectedVDB, setSelectedVDB] = useState<string | null>(null);
  ...
  const handleSaveSettings = async () => {
    setSaving(true);
    const form = document.getElementById("vectordb-form") as HTMLFormElement;
    ...
    if (error) {
      showToast(t("vector.saveFailed", { error }), "error");
      setHasChanges(true);
    } else {
