...
function availabilityLabel(
  availability: NotebookSourceProvider["availability"],
): string | null {
...
export default function AddSourceModal({
  ...
  function handleProvider(provider: NotebookSourceProvider) {
    ...
    if (provider.id === "web" || provider.id === "youtube") {
      onAddUrl();
      onClose();
      return;
    }
        ...
        <header className="flex items-center justify-between border-b border-theme-border px-5 py-4">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={onClose}
              className="hidden h-8 w-8 items-center justify-center rounded-lg border-none bg-transparent text-theme-text-secondary hover:bg-theme-bg-secondary sm:flex"
              aria-label="Zurück"
            >
              <ArrowLeft size={17} />
            </button>
            <div>
              ...
              <p className="mt-0.5 text-xs text-theme-text-secondary">
                Wähle aus, worauf dieses Notebook zugreifen darf.
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-lg border-none bg-transparent text-theme-text-secondary hover:bg-theme-bg-secondary hover:text-theme-text-primary"
        ...
        <div className="border-b border-theme-border px-5 py-4">
          <div className="flex items-center gap-2 rounded-xl border border-theme-border bg-theme-bg-secondary px-3">
            ...
            <input
              ...
              placeholder="Dateien, GitHub, Gmail, Notion …"
              className="h-10 min-w-0 flex-1 border-none bg-transparent text-sm text-theme-text-primary outline-none placeholder:text-theme-text-secondary"
            />
          </div>
        </div>
          ...
          {!query && (
            <div className="mb-3 grid grid-cols-2 gap-2">
              <button
                ...
                className="flex items-center gap-3 rounded-xl border border-theme-border bg-theme-bg-secondary px-4 py-3 text-left hover:bg-theme-bg-tertiary"
              >
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {badge && (
                        <span className="shrink-0 rounded-full bg-theme-bg-tertiary px-1.5 py-0.5 text-[9px] font-medium text-theme-text-secondary">
                          {badge}
                        </span>
                      )}
                    </div>
                    <span className="mt-0.5 block text-[11px] leading-4 text-theme-text-secondary">
