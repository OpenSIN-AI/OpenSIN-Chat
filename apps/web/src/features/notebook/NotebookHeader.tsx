  ...
  return (
    <header className="flex h-14 shrink-0 items-center border-b border-theme-border bg-theme-bg-primary pl-3 pr-14 sm:pl-5 md:pr-16">
      <div className="min-w-0 shrink">
        <h1 className="truncate text-sm font-semibold text-theme-text-primary">
          {notebookName || "Unbenanntes Notebook"}
        </h1>
      </div>
      <nav
        aria-label="Notebook-Bereiche"
        className="no-scroll ml-auto flex min-w-0 items-center gap-1 overflow-x-auto"
      >
            ...
            <button
              ...
              className={`flex h-9 items-center gap-1.5 rounded-lg border-none px-2.5 text-xs font-medium transition-colors ${
                active
                  ...
                  : "bg-transparent text-theme-text-secondary hover:bg-theme-bg-secondary hover:text-theme-text-primary"
              }`}
            >
              <Icon size={15} weight={active ? "fill" : "regular"} />
              <span className="hidden lg:inline">{section.label}</span>
              {count !== null && count > 0 && (
                <span className="rounded-full bg-theme-bg-tertiary px-1.5 py-0.5 text-[9px] text-theme-text-secondary">
                  {count}
                </span>
              )}
            </button>
