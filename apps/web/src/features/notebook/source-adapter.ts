...
function sourceUri(
  ...
  const value =
    metadata.url || metadata.sourceUrl || metadata.source || document.docpath;
  return typeof value === "string" && value.trim() ? value.trim() : undefined;
}

export function workspaceDocumentToNotebookSource(
  document: WorkspaceDocument,
  notebookSlug: string,
): NotebookSource {
...
export function workspaceDocumentsToNotebookSources(
  documents: WorkspaceDocument[] | undefined,
  notebookSlug: string,
): NotebookSource[] {
  if (!Array.isArray(documents)) return [];
  return documents.map((document) =>
    workspaceDocumentToNotebookSource(document, notebookSlug),
  );
}
