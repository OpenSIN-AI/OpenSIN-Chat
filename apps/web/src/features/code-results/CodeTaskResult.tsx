...
function FileChange({ output }: { output: CodeTaskOutput }) {
  return (
    <div className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-theme-bg-secondary">
      ...
      {!!output.deletions && (
        <span className="text-[11px] text-red-400">−{output.deletions}</span>
      )}
    </div>
  );
}
...
export default function CodeTaskResult({ outputs = [] }: CodeTaskResultProps) {
  ...
  const terminals = outputs.filter(
    (o) => o.type === "terminal" || o.type === "test-result",
  );
    ...
    <div className="flex flex-col gap-5">
      {task && (
        <section>
          ...
          <h3 className="mt-1 text-sm font-medium text-theme-text-primary">
      ...
      {files.length > 0 && (
        <section>
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-theme-text-muted">
            Geänderte Dateien
          </p>
      ...
      {summary?.content && (
        <section>
          ...
          <p className="text-sm leading-6 text-theme-text-primary">
            {summary.content}
          </p>
        </section>
      )}
