    ...
    <section className="rounded-xl border border-theme-border bg-theme-bg-secondary/50">
      ...
      >
        ...
        <span className="flex-1 text-xs font-medium text-theme-text-secondary">
          {active
            ? "Arbeitet an der Aufgabe"
            : `${messages.length} Arbeitsschritte`}
        </span>

        <CaretDown
          size={13}
          className={[
            "text-theme-text-muted transition-transform",
            open ? "rotate-180" : "",
          ].join(" ")}
        />
      </button>
          ...
          <div className="border-t border-theme-border px-3 py-2">
            {messages.map((message, index) => (
              <div
                ...
                className="flex items-start gap-2 py-1 text-xs text-theme-text-muted"
              >
