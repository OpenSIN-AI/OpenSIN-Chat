...
function describeCommand(args: Record<string, unknown>): string | null {
  return (
    safeString(args.command) || safeString(args.cmd) || safeString(args.script)
  );
}
...
function describeFile(args: Record<string, unknown>): string | null {
  return (
    ...
    safeString(args.filepath) ||
    safeString(args.filename)
  );
}
...
export function describeToolArgs(
  ...
  const specialized =
    describeSearch(args) || describeCommand(args) || describeFile(args);
