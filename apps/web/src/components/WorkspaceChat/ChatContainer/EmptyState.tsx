...
function notebookDescription(mode: "chat" | "work" | "code"): string {
  switch (mode) {
    case "work":
      return "Recherchiere, plane und erledige Aufgaben mit deinen Quellen.";
    case "code":
      return "Arbeite mit Repositories und deinen verbundenen Coding-Agenten.";
    case "chat":
    default:
      return "Frage frei oder nutze die Quellen dieses Notebooks.";
  }
}
...
export default function EmptyState({
  ...
  return (
    <section
      aria-labelledby="notebook-welcome-title"
    ...
    >
      <div className="mx-auto flex min-h-full w-full max-w-3xl flex-col justify-center px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mb-3">
          <NotebookModeCards
            value={notebookMode.modeId}
            onChange={notebookMode.setModeId}
          />
        </div>
        ...
        <div className="mt-6 flex flex-col gap-6">
          ...
          <NotebookQuickActions
            mode={notebookMode.modeId}
            onSelect={usePrompt}
          />
        </div>

        <p className="mt-8 text-center text-[10px] leading-4 text-theme-text-muted">
          OpenSIN kann Fehler machen. Prüfe wichtige Angaben und ausgeführte
          Aktionen.
        </p>
      </div>
    </section>
