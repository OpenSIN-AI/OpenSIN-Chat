...
export default function NoteEditor({
  ...
  const insertLink = useCallback(() => {
    ...
    const url = window.prompt(
      t("notepad.prompt.linkUrl", "Link-URL"),
      previous,
    );
    if (url === null) return;
  ...
  const insertMedia = useCallback(
    (kind: "image" | "youtube") => {
      ...
      const url = window.prompt(
        kind === "youtube"
          ? t("notepad.prompt.youtubeUrl", "YouTube-URL")
          : t("notepad.prompt.imageUrl", "Bild-URL"),
      );
      if (!url) return;
      if (kind === "youtube")
  ...
  const insertLinkCard = useCallback(() => {
    if (!editor) return;
    const url = window.prompt(
      t("notepad.prompt.linkPreviewUrl", "Link für Vorschaukarte"),
    );
          ...
          <div className="relative">
            ...
            {insertOpen && (
              <div className="absolute left-0 top-9 z-50 w-44 rounded-lg border border-theme-modal-border bg-theme-bg-secondary p-1 shadow-xl">
                ...
                >
                  <ImageIcon size={15} />{" "}
                  {t("notepad.insert.uploadImage", "Bild hochladen")}
                </button>
                <button
                  className="notepad-menu-item"
                ...
                >
                  <ImageIcon size={15} />{" "}
                  {t("notepad.insert.imageUrl", "Bild-URL")}
                </button>
                <button
                  className="notepad-menu-item"
                ...
                <button className="notepad-menu-item" onClick={insertLinkCard}>
                  <LinkIcon size={15} />{" "}
                  {t("notepad.insert.linkPreview", "Link-Vorschau")}
                </button>
                <button
                  className="notepad-menu-item"
                  onClick={() => {
                ...
                >
                  <Minus size={15} />{" "}
                  {t("notepad.insert.divider", "Trennlinie")}
                </button>
              </div>
            )}
          </div>
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden">
        ...
        {editable && slashOpen && (
          ...
          >
            ...
            >
              <TextHOne size={15} />{" "}
              {t("notepad.slash.heading1", "Überschrift 1")}
            </button>
            <button
              className="notepad-menu-item"
            ...
            >
              <TextHTwo size={15} />{" "}
              {t("notepad.slash.heading2", "Überschrift 2")}
            </button>
            <button
              className="notepad-menu-item"
            ...
            >
              <LinkIcon size={15} />{" "}
              {t("notepad.slash.linkPreview", "Link-Vorschau")}
            </button>
            <button
              className="notepad-menu-item"
