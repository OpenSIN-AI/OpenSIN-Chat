...
export function DnDFileUploaderProvider({
  ...
  async function handleRemoveParsedFile(event: any) {
    ...
    setFiles((prev: Attachment[]) =>
      prev.filter(
        (prevFile: Attachment) => prevFile.document?.id !== document.id,
      ),
    );
  }
  ...
  const parseAttachments = useCallback(() => {
    ...
    const imageAttachments = files
      .filter(
        (file: Attachment) => file.type === "attachment" && file.contentString,
      )
    ...
    setFiles((prev: Attachment[]) =>
      prev.map((prevFile: Attachment) => {
        const update = fileUpdates.find(
          (f: { uid: string }) => f.uid === prevFile.uid,
        );
        return update
          ? ({ ...prevFile, ...update.updates } as Attachment)
          : prevFile;
      }),
    );
    setShowWarningModal(false);
