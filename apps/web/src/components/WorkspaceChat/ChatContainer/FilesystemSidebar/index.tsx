...
export function FilesystemPanelBody({
  ...
  const filteredItems = useMemo(() => {
    ...
    return typedItems.filter((item: FilesystemItem) =>
  ...
  const fileCount = useMemo(
    () =>
      (items as FilesystemItem[]).filter(
        (i: FilesystemItem) => i.type === "file",
      ).length,
    [items],
  );
  ...
  const handleUploadFiles = useCallback(
    ...
    [apiPrefix, attachExternalFile, browse, currentPath, scope, t],
  );
  ...
  const handleToggleSelection = useCallback(
    async (item: FilesystemItem) => {
      const alreadySelected = (selectedFiles as FilesystemItem[]).some(
        (f: FilesystemItem) => f.path === item.path,
      );
  ...
  useEffect(() => {
    function onRemove(e: any) {
      ...
      for (const [path, mappedUid] of pathToUidRef.current.entries()) {
        ...
        const item = (selectedFiles as FilesystemItem[]).find(
          (f: FilesystemItem) => f.path === path,
        );
  ...
  const folders = filteredItems.filter(
    (item: FilesystemItem) => item.type === "directory",
  );
  const allFiles = filteredItems.filter(
    (item: FilesystemItem) => item.type === "file",
  );
  ...
  const uploadFiles = allFiles.filter(
    (item: FilesystemItem) => !reportFiles.includes(item),
  );
