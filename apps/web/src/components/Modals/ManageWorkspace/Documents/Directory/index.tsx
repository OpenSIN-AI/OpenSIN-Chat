...
function Directory({
    ...
    setSelectedItems((prevSelectedItems: Record<string, boolean>) => {
      ...
      if (item.type === "folder") {
        ...
        if (newSelectedItems[item.name]) {
          ...
          item.items?.forEach(
            (file: DirectoryItem) => delete newSelectedItems[file.id],
          );
        } else {
          newSelectedItems[item.name] = true;
          item.items?.forEach(
            (file: DirectoryItem) => (newSelectedItems[file.id] = true),
          );
        }
      } else {
  ...
  const isSelected = (id: string, item: DirectoryItem | null) => {
    if (item && item.type === "folder") {
      ...
      return (item.items ?? []).every(
        (file: DirectoryItem) => selectedItems[file.id],
      );
    }
  ...
  const totalDocCount = (files?.items ?? []).reduce(
    (acc: number, folder: DirectoryItem) => {
