...
export default function DocumentSettings({ workspace }: DocumentSettingsProps) {
  ...
  const [availableDocs, setAvailableDocs] = useState<DocsResponse>({
    items: [],
  });
  const [loading, setLoading] = useState(true);
  const [workspaceDocs, setWorkspaceDocs] = useState<DocsResponse>({
    items: [],
  });
  ...
  const [movedItems, setMovedItems] = useState<
    (DocumentFile & { folderName: string })[]
  >([]);
  ...
  useEffect(() => {
    ...
    const documentsInWorkspace =
      currentWorkspace.documents.map(
        (doc: { docpath: string }) => doc.docpath,
      ) || [];
  ...
  const moveSelectedItemsToWorkspace = () => {
    ...
    for (const itemId of Object.keys(selectedItems)) {
      for (const folder of availableDocs.items) {
        const foundItem = folder.items.find(
          (file: DocumentFile) => file.id === itemId,
        );
        if (foundItem) {
    ...
    const newAvailableDocs: DocsResponse = JSON.parse(
      JSON.stringify(availableDocs),
    );
    const newWorkspaceDocs: DocsResponse = JSON.parse(
