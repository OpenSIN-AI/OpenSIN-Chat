...
async function asPdf({
  ...
  try {
    ...
    docs = [];
    console.error(
      `[asPDF] Native PDF parsing failed for ${filename}: ${e.message}. Attempting OCR recovery.`,
    );
  }
  ...
  if (docs.length === 0) {
    ...
    try {
      ...
      return {
        success: false,
        reason: nativeParseError
          ? `Native PDF parsing failed (${nativeParseError.message}) and OCR failed: ${e.message}`
          : `No text content found in ${filename} and OCR failed: ${e.message}`,
        documents: [],
      };
    }
  } else {
