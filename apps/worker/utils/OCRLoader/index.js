...
const { UnlimitedOcrClient } = require("./unlimited");

const execFileAsync = promisify(execFile);
const OCR_MAX_BYTES = 5 * 1024 * 1024 * 1024;
const UNLIMITED_OCR_PDF_DPI = Number(process.env.UNLIMITED_OCR_PDF_DPI || 300);
const UNLIMITED_OCR_RENDER_TIMEOUT_MS = Number(
  process.env.UNLIMITED_OCR_RENDER_TIMEOUT_MS || 300_000,
);
...
async function readPdfInfoWithPoppler(filePath) {
  ...
  if (!Number.isInteger(totalPages) || totalPages <= 0) {
    throw new Error("pdfinfo returned no valid page count");
  }
  return { totalPages, info };
}

async function renderPdfRangeWithPdftoppm(filePath, pageNumbers) {
  ...
  const tempDir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "unlimited-ocr-"),
  );
  ...
  try {
    await execFileAsync(
      ...
      [
        ...
        "-l",
        String(pageNumbers[pageNumbers.length - 1]),
        "-r",
        String(UNLIMITED_OCR_PDF_DPI),
        "-png",
        filePath,
        outputPrefix,
      ],
      {
        timeout: UNLIMITED_OCR_RENDER_TIMEOUT_MS,
    ...
    if (renderedFiles.length !== pageNumbers.length) {
      throw new Error(
        `pdftoppm rendered ${renderedFiles.length}/${pageNumbers.length} pages`,
      );
    }
    return await Promise.all(
      renderedFiles.map((name) =>
        fs.promises.readFile(path.join(tempDir, name)),
      ),
    );
  } finally {
...
const NIM_CONCURRENCY = Math.max(
  1,
  Number(process.env.NVIDIA_NIM_CONCURRENCY || 4) || 4,
);
...
const SUPPORTED_OCR_ENGINES = new Set([
  "auto",
  "unlimited",
  "nim",
  "tesseract",
]);
const requestedOcrEngine = String(
  process.env.OCR_ENGINE || "auto",
).toLowerCase();
const OCR_ENGINE = SUPPORTED_OCR_ENGINES.has(requestedOcrEngine)
  ? requestedOcrEngine
  : "auto";
const UNLIMITED_OCR_CONFIGURED = Boolean(
  process.env.UNLIMITED_OCR_BASE_URL ||
  String(process.env.UNLIMITED_OCR_AUTO_DISCOVER || "").toLowerCase() ===
    "true",
);

function shouldTryUnlimited() {
  return (
    OCR_ENGINE === "unlimited" ||
    (OCR_ENGINE === "auto" && UNLIMITED_OCR_CONFIGURED)
  );
}

function shouldTryNim() {
  return OCR_ENGINE === "nim" || OCR_ENGINE === "auto";
}
...
class OCRLoader {
  ...
  constructor({ targetLanguages = "eng" } = {}) {
    ...
    this.unlimitedClient = new UnlimitedOcrClient({
      logger: (message) => this.log(message),
    });

    const autoChain = [
      UNLIMITED_OCR_CONFIGURED ? "Unlimited-OCR" : null,
      NIM_API_KEY ? "NVIDIA NIM" : null,
      "tesseract.js",
    ]
      .filter(Boolean)
      .join(" -> ");
    const engineLabel =
      OCR_ENGINE === "auto"
        ? `auto (${autoChain})`
        : OCR_ENGINE === "unlimited"
          ? `Unlimited-OCR (${this.unlimitedClient.model} at ${this.unlimitedClient.baseUrl})`
          : OCR_ENGINE === "tesseract"
            ? "tesseract.js"
            : NIM_API_KEY
              ? `NVIDIA NIM (${NIM_MODEL})`
              : "NVIDIA NIM (not configured; will fall back to tesseract.js)";
  ...
  async ocrPDF(
    ...
    // Prefer complete 300-DPI Poppler page rendering for every OCR provider.
    // This keeps valid modern PDFs independent from the legacy PDF.js 2.0.550
    // compatibility parser, including when a remote OCR provider is down.
    try {
      const { totalPages, info } = await readPdfInfoWithPoppler(filePath);
      const popplerMetadata = {
        source: filePath,
        pdf: {
          version: info["PDF version"] || "poppler",
          info,
          metadata: null,
          totalPages,
        },
      ...
      if (shouldTryUnlimited()) {
        const unlimitedOk = await this.unlimitedClient.isAvailable();
        if (unlimitedOk) {
          try {
            const documents = await this._ocrPdfWithUnlimited(
              filePath,
              { numPages: totalPages },
              null,
              popplerMetadata,
            ...
            this.log(
              "Unlimited-OCR Poppler path returned no usable pages; trying the next provider.",
            );
          } catch (error) {
            this.log(
              `Unlimited-OCR Poppler path failed: ${error.message}; trying the next provider.`,
            );
          }
        } else {
          this.log(
            `Unlimited-OCR endpoint ${this.unlimitedClient.baseUrl} is unavailable; trying the next provider.`,
          );
        }
      }
      ...
      if (shouldTryNim()) {
        ...
        if (nimOk) {
          try {
            const documents = await this._ocrPdfWithNimPoppler(
              filePath,
              totalPages,
              popplerMetadata,
              documentTitle,
            ...
            if (documents.length > 0) return documents;
            this.log(
              "NVIDIA NIM Poppler path returned no usable pages; falling back to Tesseract.",
            );
          } catch (error) {
            this.log(
              `NVIDIA NIM Poppler path failed: ${error.message}; falling back to Tesseract.`,
            );
          }
        } else {
          this.log(
      ...
      try {
        const documents = await this._ocrPdfWithTesseractPoppler(
          filePath,
          totalPages,
          popplerMetadata,
          documentTitle,
        ...
        if (documents.length > 0) return documents;
        this.log(
          "Tesseract Poppler path returned no usable pages; trying the PDF.js compatibility path.",
        );
      } catch (error) {
        this.log(
          `Tesseract Poppler path failed: ${error.message}; trying the PDF.js compatibility path.`,
        );
      }
    } catch (error) {
      this.log(
        `Poppler PDF inspection failed: ${error.message}; trying the PDF.js compatibility path.`,
      );
    }
    ...
    // ─── Baidu Unlimited-OCR path ────────────────────────────────
    if (shouldTryUnlimited()) {
      const unlimitedOk = await this.unlimitedClient.isAvailable();
      if (unlimitedOk) {
        try {
          const unlimitedDocuments = await this._ocrPdfWithUnlimited(
            filePath,
            pdfDocument,
            pdfjs,
            metadata,
          ...
          if (unlimitedDocuments.length > 0) return unlimitedDocuments;
          this.log(
            "Unlimited-OCR returned no usable pages, falling back to tesseract.js",
          );
        } catch (error) {
          this.log(
            `Unlimited-OCR failed: ${error.message}; falling back to tesseract.js`,
          );
        }
      } else {
        this.log(
          `Unlimited-OCR endpoint ${this.unlimitedClient.baseUrl} is unavailable, falling back to tesseract.js`,
        );
      }
    }
  ...
  /**
   * Layout-aware, multi-page PDF OCR via Baidu Unlimited-OCR.
   * Complete pages are rendered at 300 DPI through pdftoppm. If Poppler is
   * unavailable, the existing embedded-image renderer is used as a safe
   * compatibility fallback.
   */
  async _ocrPdfWithUnlimited(
    filePath,
    pdfDocument,
    pdfjs,
    metadata,
    ...
    const documents = [];
    const pagesPerRequest = this.unlimitedClient.pagesPerRequest;
    ...
    this.log(
      `Using Unlimited-OCR (${this.unlimitedClient.model}) for ${totalPages} pages in chunks of ${pagesPerRequest}`,
    );
    ...
    for (
      ...
      offset += pagesPerRequest
    ) {
      if (Date.now() - startTime > maxExecutionTime) {
        this.log(
          `Unlimited-OCR timeout reached at page ${offset + 1}/${totalPages}, returning partial results`,
        );
        break;
      }

      const chunkPages = pageNumbers.slice(offset, offset + pagesPerRequest);
      let imageBuffers;
      try {
        imageBuffers = await renderPdfRangeWithPdftoppm(filePath, chunkPages);
      } catch (renderError) {
        if (!pdfjs || typeof pdfDocument?.getPage !== "function") {
          throw renderError;
        }
        this.log(
          `Full-page ${UNLIMITED_OCR_PDF_DPI} DPI render unavailable for pages ${chunkPages.join(
            "-",
          )}: ${renderError.message}. Falling back to embedded images.`,
        );
        if (!pdfSharp) {
        ...
        imageBuffers = await Promise.all(
          chunkPages.map(async (pageNumber) => {
            const page = await pdfDocument.getPage(pageNumber);
            return await pdfSharp.pageToBuffer({ page });
          }),
        );
      }
      ...
      const validPages = [];
      const validBuffers = [];
      for (let index = 0; index < chunkPages.length; index += 1) {
        if (!imageBuffers[index]) continue;
        validPages.push(chunkPages[index]);
        validBuffers.push(imageBuffers[index]);
      }
      if (!validBuffers.length) continue;

      const texts = await this.unlimitedClient.recognizePages(validBuffers, {
      ...
      for (let index = 0; index < validPages.length; index += 1) {
        ...
        documents.push({
          ...
          metadata: {
            ...metadata,
            loc: { pageNumber: validPages[index] },
            ocr: {
              engine: "unlimited-ocr",
              model: this.unlimitedClient.model,
              multiPage: validPages.length > 1,
              dpi: UNLIMITED_OCR_PDF_DPI,
            },
          },
        });
        this.log(`✅ Unlimited-OCR completed pg${validPages[index]}`);
      }
    }

    this.log(`Completed Unlimited-OCR of ${documentTitle}!`, {
  ...
  /**
   * OCR complete Poppler-rendered PDF pages through NVIDIA NIM.
   */
  async _ocrPdfWithNimPoppler(
    filePath,
    totalPages,
    metadata,
    documentTitle,
    ...
    this.log(
      `Using NVIDIA NIM Vision (${NIM_MODEL}) on ${totalPages} Poppler-rendered pages`,
    );

    for (
      let offset = 0;
      offset < pageNumbers.length;
      offset += NIM_CONCURRENCY
    ) {
      if (Date.now() - startTime > maxExecutionTime) {
        this.log(
          `NIM OCR timeout reached at page ${offset + 1}/${totalPages}, returning partial results`,
        );
        break;
      }

      const batchPages = pageNumbers.slice(offset, offset + NIM_CONCURRENCY);
      const imageBuffers = await renderPdfRangeWithPdftoppm(
        filePath,
        batchPages,
      );
      const batchResults = await Promise.all(
        imageBuffers.map(async (imageBuffer, index) => {
          ...
          try {
            ...
            this.log(`✅ NIM OCR completed pg${pageNumber}`);
            return {
              pageContent: text,
              metadata: {
                ...metadata,
            ...
            this.log(`NIM OCR error on pg${pageNumber}: ${error.message}`);
            return null;
          }
        }),
      );
      documents.push(...batchResults.filter(Boolean));
    }
  ...
  /**
   * OCR complete Poppler-rendered PDF pages through local Tesseract workers.
   */
  async _ocrPdfWithTesseractPoppler(
    filePath,
    totalPages,
    metadata,
    documentTitle,
    ...
    const effectiveBatchSize = Math.max(1, Number(batchSize) || 10);
    const workerCount = Math.max(
      1,
      Math.min(maxWorkers ?? os.cpus().length, 4),
    );
    const startTime = Date.now();
    const documents = [];
    let workerPool = [];
    ...
    try {
      ...
      this.log("Bootstrapping Poppler/Tesseract OCR completed!", {
        MAX_EXECUTION_TIME_MS: maxExecutionTime,
        BATCH_SIZE: effectiveBatchSize,
        MAX_CONCURRENT_WORKERS: workerPool.length,
      ...
      const processPages = async () => {
        for (
          ...
          startPage += effectiveBatchSize
        ) {
          const endPage = Math.min(
            startPage + effectiveBatchSize - 1,
            totalPages,
          );
          const pageNumbers = Array.from(
          ...
          const queue = pageNumbers.map((pageNumber, index) => ({
            pageNumber,
              ...
              while (queue.length) {
                ...
                try {
                  ...
                  batchDocuments.push({
                    ...
                    metadata: {
                      ...metadata,
                      loc: { pageNumber: item.pageNumber },
                      ocr: {
                        engine: "tesseract",
                        dpi: UNLIMITED_OCR_PDF_DPI,
                      },
                  ...
                  this.log(
                    `[Worker ${workerIndex + 1}] error on pg${item.pageNumber}: ${error.message}`,
                  );
                }
              }
            }),
      ...
      await Promise.all(
        workerPool.map((worker) =>
          worker
            .terminate()
            .catch((error) =>
              this.log(`Tesseract worker termination failed: ${error.message}`),
            ),
        ),
      );
    }

    this.log(`Completed Tesseract OCR of ${documentTitle}!`, {
      documentsParsed: documents.length,
      totalPages,
      executionTime: `${((Date.now() - startTime) / 1000).toFixed(2)}s`,
    });
  ...
  async ocrImage(filePath, { maxExecutionTime = 300_000 } = {}) {
    ...
    try {
      ...
      // Apply EXIF orientation before sending an image to a vision model.
      const readOrientedImage = async () => {
      ...
      if (shouldTryUnlimited()) {
        ...
        if (unlimitedOk) {
          try {
            const text = await this.unlimitedClient.recognizeImage(
              await readOrientedImage(),
            );
            if (text) {
              this.log(`Completed Unlimited-OCR of ${documentTitle}!`, {
                executionTime: `${((Date.now() - startTime) / 1000).toFixed(2)}s`,
            ...
            this.log(
              `Unlimited-OCR image request failed: ${error.message}; falling back to tesseract.js`,
            );
          }
        } else {
          this.log(`Unlimited-OCR not available, falling back to tesseract.js`);
        }
      }
      ...
      if (shouldTryNim()) {
        const nimOk = await isNimAvailable();
        if (nimOk) {
          const text = await nimOcrImage(await readOrientedImage());
          this.log(`Completed NIM OCR of ${documentTitle}!`, {
            executionTime: `${((Date.now() - startTime) / 1000).toFixed(2)}s`,
          });
          return text || "";
  ...
  async ocrImageBatch(filePaths = [], { maxExecutionTime = 600_000 } = {}) {
    ...
    // Read and orient all images once for remote vision providers.
    const readImageBuffers = async () =>
      await Promise.all(
        valid.map(async (idx) => {
          try {
    ...
    if (shouldTryUnlimited()) {
      ...
      if (unlimitedOk) {
        try {
          const imageBuffers = await readImageBuffers();
          const present = imageBuffers
            .map((buffer, index) => ({ buffer, index }))
            .filter(({ buffer }) => buffer);
          const texts = await this.unlimitedClient.recognizePages(
            present.map(({ buffer }) => buffer),
            { pageNumbers: present.map(({ index }) => index + 1) },
          );
          present.forEach(({ index }, resultIndex) => {
            results[valid[index]] = texts[resultIndex] || "";
          });
          this.log(`Completed Unlimited-OCR image batch.`, {
          ...
          this.log(
            `Unlimited-OCR batch failed: ${error.message}; falling back to tesseract.js`,
          );
        }
      } else {
        this.log(`Unlimited-OCR not available, falling back to tesseract.js`);
