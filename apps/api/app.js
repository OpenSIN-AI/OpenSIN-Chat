...
// Purpose: Express app factory for the API application.
// Docs: docs/architecture.md
// This file exports the app factory so that both the production entry point
// (apps/api/index.js) and the Vitest test suites in tests/ can build the same
...
function buildApp() {
  ...
  app.use((request, response, next) => {
    ...
    const isPublicEmbedRequest =
      request.path === "/api/embed" || request.path.startsWith("/api/embed/");
  ...
  if (process.env.NODE_ENV === "development" && envFlag("ENABLE_HTTP_LOGGER")) {
    app.use(
      httpLogger({
  ...
  if (
    ...
    //
    // Prefer precompressed .br/.gz companions (see apps/web postbuild) so the
 ...
 * encryption, background workers, etc.). Used by apps/api/index.js.
 */
