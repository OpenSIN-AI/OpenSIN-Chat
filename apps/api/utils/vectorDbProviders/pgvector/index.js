...
class PGVector extends VectorDatabase {
   ...
   * - Defaults to "opensin_vectors" if no table name is provided.
   * @returns {string}
   */
  static tableName() {
    return process.env.PGVECTOR_TABLE_NAME || "opensin_vectors";
  }
   ...
   * so that opensin can manage the table directly.
   *
  ...
  /**
   * Check if the opensin embedding table exists in the database
   * @returns {Promise<boolean>}
   */
  async dbTableExists() {
    let connection = null;
  ...
  /**
   * Reset the entire vector database table associated with opensin
