...
function inProcessPluginExecutionEnabled() {
  if (process.env.NODE_ENV === "production") return false;
  return ["1", "true", "yes", "on"].includes(
    String(process.env.ENABLE_UNSAFE_IN_PROCESS_PLUGINS || "")
      .trim()
      .toLowerCase(),
  );
}
...
class ImportedPlugin {
  constructor(config) {
    ...
    this.executionEnabled = inProcessPluginExecutionEnabled();
    if (!this.allowlisted || !this.executionEnabled) {
      const reason = !this.allowlisted
        ? "not on config/plugins-allowlist.toml"
        : "in-process execution is disabled";
      consoleLogger.log(
        `ImportedPlugin — hubId '${this.config.hubId}' ${reason}; skipping handler load`,
      );
      this.handler = null;
  ...
  static activeImportedPlugins() {
    ...
    for (const folder of folders) {
      ...
      if (
        config.active &&
        isAllowlisted(config.hubId) &&
        inProcessPluginExecutionEnabled()
      )
  ...
  static listImportedPlugins() {
    ...
    for (const folder of folders) {
      ...
      config.executionEnabled = inProcessPluginExecutionEnabled();
      plugins.push(config);
    }
    return plugins;
  }
...
module.exports.inProcessPluginExecutionEnabled =
  inProcessPluginExecutionEnabled;
