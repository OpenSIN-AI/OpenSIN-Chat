...
const { documentsPath, isWithin, normalizePath } = require("../files");
...
async function addArtifactAsWorkspaceSource({
  ...
  if (!artifact || !workspace)
    throw new Error("Artifact and workspace are required");
  ...
  try {
    ...
    if (!document) {
      throw new Error(
        "Artifact was embedded but no document record was created",
      );
    }
