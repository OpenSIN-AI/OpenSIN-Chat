 ...
 * the mime type for documents is `application/opensin-document` - anything else is assumed to be an image.
...
async function processDocumentAttachments(attachments = []) {
  ...
  for (const attachment of attachments) {
    if (
      ...
      attachment.mime &&
      attachment.mime.toLowerCase() === "application/opensin-document"
    )
      ...
      .then(async ({ thoughts, textResponse, outputs, metrics }) => {
        ...
        if (allOutputs.length > 0) {
          ...
          createArtifactsFromOutputs({
            ...
            consoleLogger.error(
              "[API Chat] Artifact creation failed:",
              err.message,
            ),
          );
        }
      ...
      .then(async ({ thoughts, textResponse, outputs, metrics }) => {
        ...
        if (allOutputs.length > 0) {
          ...
          createArtifactsFromOutputs({
            ...
            consoleLogger.error(
              "[API Chat Stream] Artifact creation failed:",
              err.message,
            ),
          );
        }
