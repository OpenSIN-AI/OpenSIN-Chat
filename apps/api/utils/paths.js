...
// Purpose: Centralized API and document-worker path helpers.
...
// NOTE: __dirname here is <repo>/apps/api/utils. Local fallbacks are resolved
// inside apps/api and apps/worker; container paths preserve /app/server and
// /app/collector for runtime compatibility.
...
function getStoragePath(...subdirs) {
  ...
  if (!path.isAbsolute(candidate)) {
    if (!pathsWarnedAboutFallback) {
      consoleLogger.warn(
        `[paths] STORAGE_DIR="${candidate}" is not absolute — falling back to local <repo>/apps/api/storage. Set STORAGE_DIR to an absolute path in .env.development to silence this warning.`,
      );
  ...
  try {
    ...
    if (!pathsWarnedAboutFallback) {
      consoleLogger.warn(
        `[paths] STORAGE_DIR="${candidate}" is not writable (${err.code || err.message}) — falling back to local <repo>/apps/api/storage. Set STORAGE_DIR in .env.development to an absolute writable path to silence this warning.`,
      );
      pathsWarnedAboutFallback = true;
    }
    process.env.STORAGE_DIR = "";
