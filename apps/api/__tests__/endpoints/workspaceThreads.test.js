...
describe("workspaceThreadEndpoints", () => {
  ...
  describe("POST /workspace/:slug/thread/new", () => {
    it("creates a thread", async () => {
      WorkspaceThread.new.mockResolvedValue({
        thread: { id: 10, slug: "t1" },
        message: null,
      });
      const res = await app.call("post", "/workspace/ws/thread/new", {
        locals: WS_LOCALS,
      });
    ...
    it("assigns pending parsed files to the newly-created thread", async () => {
      WorkspaceThread.new.mockResolvedValue({
        thread: { id: 10, slug: "t1" },
        message: null,
      });
      WorkspaceParsedFiles.assignToThread.mockResolvedValue({
        success: true,
        assignedCount: 2,
        error: null,
      });
      ...
      const res = await app.call("post", "/workspace/ws/thread/new", {
        body: { parsedFileIds: [4, 5] },
        locals: WS_LOCALS,
      });

      expect(res.statusCode).toBe(200);
      expect(WorkspaceParsedFiles.assignToThread).toHaveBeenCalledWith({
        fileIds: [4, 5],
        workspaceId: 1,
        threadId: 10,
        userId: null,
    ...
    it("rolls back the empty thread when parsed-file assignment fails", async () => {
      WorkspaceThread.new.mockResolvedValue({
        thread: { id: 10, slug: "t1" },
        message: null,
      });
      ...
      const res = await app.call("post", "/workspace/ws/thread/new", {
        body: { parsedFileIds: [999] },
        locals: WS_LOCALS,
      });

      expect(res.statusCode).toBe(409);
      expect(res.body.thread).toBeNull();
      expect(WorkspaceThread.delete).toHaveBeenCalledWith({ id: 10 });
    });

    it("rejects malformed parsed-file IDs before creating a thread", async () => {
      const res = await app.call("post", "/workspace/ws/thread/new", {
        body: { parsedFileIds: ["bad"] },
        locals: WS_LOCALS,
      });
  ...
  describe("GET /workspace/:slug/threads", () => {
    it("returns threads and folders", async () => {
      ...
      const res = await app.call("get", "/workspace/ws/threads", {
        locals: WS_LOCALS,
      });
      expect(res.statusCode).toBe(200);
  ...
  describe("DELETE /workspace/:slug/thread/:threadSlug", () => {
    it("deletes a thread", async () => {
      ...
      const res = await app.call("delete", "/workspace/ws/thread/t1", {
  ...
  describe("GET /workspace/:slug/thread/:threadSlug/chats", () => {
    it("returns thread chat history", async () => {
      ...
      const res = await app.call("get", "/workspace/ws/thread/t1/chats", {
        locals: THREAD_LOCALS,
      });
      expect(res.statusCode).toBe(200);
  ...
  describe("DELETE /workspace/:slug/thread/:threadSlug/delete-edited-chats", () => {
    it("deletes chats from startingId", async () => {
      ...
      const res = await app.call(
        "delete",
        "/workspace/ws/thread/t1/delete-edited-chats",
        {
          body: { startingId: 50 },
          locals: THREAD_LOCALS,
        },
  ...
  describe("POST /workspace/:slug/thread/:threadSlug/update-chat", () => {
    it("updates assistant chat text", async () => {
      ...
      const res = await app.call(
        "post",
        "/workspace/ws/thread/t1/update-chat",
        {
    ...
    it("updates user chat text", async () => {
      ...
      expect(WorkspaceChats._update).toHaveBeenCalledWith(1, {
        prompt: "new prompt",
      });
    });
  ...
  describe("POST /workspace/:slug/thread-folder/new", () => {
    it("creates a folder", async () => {
      WorkspaceThreadFolder.new.mockResolvedValue({
        folder: { id: 1, name: "F1" },
        message: null,
      });
  ...
  describe("POST /workspace/:slug/thread-folder/:folderId/update", () => {
    it("updates a folder", async () => {
      WorkspaceThreadFolder.update.mockResolvedValue({
        folder: { id: 1, name: "Updated" },
        message: null,
      });
      const res = await app.call(
        "post",
        "/workspace/ws/thread-folder/1/update",
        {
          body: { name: "Updated" },
          locals: WS_LOCALS,
        },
  ...
  describe("DELETE /workspace/:slug/thread-folder/:folderId", () => {
    it("deletes a folder", async () => {
      ...
      const res = await app.call("delete", "/workspace/ws/thread-folder/1", {
        locals: WS_LOCALS,
      });
      expect(res.statusCode).toBe(200);
    });
  ...
  describe("POST /workspace/:slug/thread/:threadSlug/assign-folder", () => {
    it("assigns a folder to a thread", async () => {
      ...
      const res = await app.call(
        "post",
        "/workspace/ws/thread/t1/assign-folder",
        {
          body: { folderId: 5 },
          locals: THREAD_LOCALS,
        },
