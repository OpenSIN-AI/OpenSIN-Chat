...
function groupValue(input = {}) {
  return {
    id: String(input.id || crypto.randomUUID())
      ...
      .slice(0, 120),
    name: String(input.name || "Neue Gruppe")
      .trim()
      .slice(0, 120),
...
function parseWorkflow(job) {
  ...
  try {
    const meta = JSON.parse(
      Buffer.from(job.prompt.slice(MARKER.length, end), "base64url").toString(
        "utf8",
      ),
    );
    return {
...
function emailAutomationEndpoints(app) {
  ...
  app.get(
    "/email-automation/bootstrap",
    [validatedRequest, emailRole],
    async (_req, res) => {
      try {
        ...
        return res.json({
          runtime: {
            ...
            error: config.runtimeError || "",
          },
          accounts: config.accounts.map((a) =>
            safeAccount(a, config.defaultAccountId),
          ),
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      const query = String(req.query.query || "is:inbox")
        .trim()
        .slice(0, 500);
  ...
  app.get(
    "/email-automation/threads/:threadId",
    [validatedRequest, emailRole],
    async (req, res) => {
      const accountId = String(req.query.accountId || "").trim() || null;
      ...
      try {
        ...
        if (!id) {
          return res.status(400).json({
            error: "Der Himalaya-Kontoalias ist erforderlich.",
          });
        }
        ...
        if (!existing) {
          return res.status(400).json({
            error:
              `Konto "${id}" ist nicht in ${config.configPath || "der Himalaya-Konfiguration"} vorhanden. ` +
              "Richte es zuerst mit sin-gmail beziehungsweise `himalaya account configure` ein.",
          });
        }
        const account = {
          ...existing,
          label: String(body.label || existing.label || existing.email || id)
            .trim()
            .slice(0, 120),
        ...
        const defaultAccountId = body.isDefault
          ? id
          : config.defaultAccountId || id;
        const result = await GmailBridge.updateConfig({
          ...config,
          accounts,
          defaultAccountId,
        });
        ...
        return res.json({
          account: safeAccount(account, defaultAccountId),
          defaultAccountId,
        });
      } catch (error) {
        return res.status(500).json({ error: error.message });
  ...
  app.post(
    "/email-automation/accounts/:id/test",
    [validatedRequest, emailRole],
    async (req, res) => {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const body = reqBody(req);
        const meta = {
          name: String(body.name || "E-Mail Workflow")
            .trim()
            .slice(0, 160),
          ...
          groupMembers: Array.isArray(body.groupMembers)
            ? body.groupMembers.map(String)
            : [],
          ...
          safetyMode: ["notify", "approval", "automatic"].includes(
            body.safetyMode,
          )
            ? body.safetyMode
            : "approval",
        ...
        if (!meta.name || !ScheduledJob.isValidCron(schedule)) {
          return res
            .status(400)
            .json({ error: "Name oder Zeitplan ist ungültig." });
        }
        ...
        if (meta.accountIds.length === 0) {
          return res
            .status(400)
            .json({ error: "Mindestens ein Gmail-Konto ist erforderlich." });
        }
        ...
        return res
          ...
          .json({ workflow: parseWorkflow(result.job) });
      } catch (error) {
        return res.status(500).json({ error: error.message });
      }
    },
  ...
  app.post(
    "/email-automation/workflows/:id/:action",
    [validatedRequest, emailRole],
    async (req, res) => {
      try {
        ...
        if (req.params.action === "toggle") {
          const result = await ScheduledJob.update(id, {
            enabled: !job.enabled,
          });
