...
function validateContent(content) {
  ...
  if (Buffer.byteLength(content, "utf-8") > MAX_ARTIFACT_SIZE_BYTES) {
    throw new ArtifactValidationError(
      "content exceeds the artifact size limit",
    );
  }
  return content;
}
...
function serializeOptionalJson(value, field) {
  ...
  if (Buffer.byteLength(serialized, "utf-8") > MAX_ARTIFACT_SIZE_BYTES) {
    throw new ArtifactValidationError(
      `${field} exceeds the artifact size limit`,
    );
  }
  return serialized;
}
...
async function listArtifacts(
  ...
  if (chatId !== undefined) where.chatId = validateOptionalId(chatId, "chatId");
...
async function updateArtifact(uuid, workspaceId, patch = {}) {
  ...
  if (patch.title !== undefined)
    data.title = validateRequiredString(patch.title, "title", MAX_TITLE_LENGTH);
  ...
  if (patch.content !== undefined)
    data.content = validateContent(patch.content);
