...
const ScheduledJob = {
  ...
  update: async function (id, data = {}) {
    try {
      ...
      if (updates.schedule) {
        if (!this.isValidCron(updates.schedule)) {
          throw new Error("Invalid cron expression");
        }
  ...
  allEnabled: async function () {
    try {
      ...
      consoleLogger.error(
        "Failed to get enabled scheduled jobs:",
        error.message,
      );
      return [];
    }
  ...
  countActive: async function (excludeId = null) {
    try {
      ...
      consoleLogger.error(
        "Failed to count active scheduled jobs:",
        error.message,
      );
      return 0;
    }
  ...
  availableTools: async function () {
    ...
    const gmailNeedsSetup = !(
      gmailConfig.binaryAvailable &&
      Array.isArray(gmailConfig.accounts) &&
      gmailConfig.accounts.some((account) => account.enabled !== false)
    );
    ...
    try {
      ...
      consoleLogger.error(
        "Failed to load MCP servers for available tools:",
        error,
      );
    }
