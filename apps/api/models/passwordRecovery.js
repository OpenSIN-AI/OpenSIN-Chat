...
const PasswordResetToken = {
  ...
  create: async function (userId) {
    try {
      const token = v4();
      const stored = await prisma.password_reset_tokens.create({
        data: {
          user_id: userId,
      ...
      return { passwordResetToken: { ...stored, token }, error: null };
    } catch (error) {
      consoleLogger.error(
  ...
  findUnique: async function (clause = {}) {
    try {
      if (
        typeof clause.token === "string" &&
        !clause.token.startsWith(TOKEN_HASH_PREFIX)
      ) {
        const presented = clause.token;
        let record = await prisma.password_reset_tokens.findUnique({
          where: { token: hashToken(presented) },
        });
        if (!record) {
          const legacy = await prisma.password_reset_tokens.findUnique({
            where: { token: presented },
          });
          if (legacy) {
            record = await prisma.password_reset_tokens.update({
              where: { id: legacy.id },
              data: { token: hashToken(presented) },
            });
          }
        }
        return record;
      }
      return await prisma.password_reset_tokens.findUnique({ where: clause });
    } catch (error) {
      consoleLogger.error(
