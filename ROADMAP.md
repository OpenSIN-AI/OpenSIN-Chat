...
| 1.5 | `@opensin/*` → `@mintplex-labs/*` Revert | ✅ | Part of server smoke test |
...
| 2.71 | cvoice.ai TTS-Provider (6. Engine) | ✅ | `server/utils/TextToSpeech/cvoice/` + `CvoiceOptions` UI + EN/DE i18n; API-Key serverseitig, gratis-Tier mit 256-Entry-LRU-Cache |
...
|---|---|---|
| `architecture.md` | Mac als Prod-Host, `opensin.db`, `config-opensin.yml` — Prod ist jetzt OCI VM `sin-supabase` | YES |
| `api.md` | `opensin` container ref, fehlende Endpoints (add-to-workspace, embed, pdf-analysis) | YES |
