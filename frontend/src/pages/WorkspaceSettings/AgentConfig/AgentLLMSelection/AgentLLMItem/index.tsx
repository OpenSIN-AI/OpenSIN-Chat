...
type LLMProvider = {
  ...
  options: (settings: any, opts: any) => JSX.Element;
};

type AgentLLMItemProps = {
  llm: LLMProvider;
          ...
          {checked &&
            ...
            !NO_SETTINGS_NEEDED.includes(value) && (
              <button
                ...
                className="border-none p-2 text-theme-text-secondary hover:text-theme-text-primary light:hover:text-theme-text-primary hover:bg-theme-bg-hover rounded-md transition-all duration-300"
...
function SetupProvider({
  ...
  postSubmit,
  settings,
}: SetupProviderProps): JSX.Element | null {
  const { t } = useTranslation();
      ...
      <div className="relative w-full max-w-2xl bg-theme-bg-secondary rounded-lg shadow border-2 border-theme-modal-border">
        <div className="relative p-6 border-b rounded-t border-theme-modal-border">
          <div className="w-full flex gap-x-2 items-center">
            <h3 className="text-xl font-semibold text-theme-text-primary overflow-hidden overflow-ellipsis whitespace-nowrap">
              {t("agentLLMItem.settingsTitle", { name: LLMOption.name })}
            </h3>
          </div>
          <button
            onClick={closeModal}
        ...
        <form id="provider-form" onSubmit={handleUpdate}>
          ...
          <div className="flex justify-between items-center mt-6 pt-6 border-t border-theme-modal-border px-7 pb-6">
            ...
            >
              {t("agentLLMItem.saveSettings", { name: LLMOption.name })}
            </button>
          </div>
        </form>
      </div>
