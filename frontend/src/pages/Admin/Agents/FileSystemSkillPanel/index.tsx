...
// Purpose: Skill panel for configuring which file-system sub-skills are enabled.
...
interface FileSystemSkillPanelProps {
  title: string;
  skill: string;
...
export default function FileSystemSkillPanel({
  ...
  setHasChanges,
  hasChanges = false,
}: FileSystemSkillPanelProps): JSX.Element {
  const { t } = useTranslation();
  const [disabledSubSkills, setDisabledSubSkills] = useState<string[]>([]);
  ...
  function toggleSubSkill(subSkillName: string) {
    setHasChanges?.(true);
