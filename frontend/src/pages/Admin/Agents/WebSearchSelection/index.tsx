...
// Purpose: Skill panel for selecting and configuring the agent's web search provider.
// Docs: WebSearchSelection.doc.md
...
import type { SearchProviderSettings } from "./SearchProviderOptions";

interface SearchProvider {
  name: string;
  value: string;
  logo: string;
  options: (settings: SearchProviderSettings) => React.ReactNode;
  description: string;
}

interface AgentWebSearchSelectionProps {
  skill?: string;
  title?: string;
  description?: string;
...
export default function AgentWebSearchSelection({
  ...
  enabled = false,
  setHasChanges,
}: AgentWebSearchSelectionProps): JSX.Element {
  const { t } = useTranslation();
  const searchInputRef = useRef<HTMLInputElement | null>(null);
  const { provider: initialProvider } = useAgentSearchProvider();
  const [filteredResults, setFilteredResults] = useState<SearchProvider[]>([]);
  ...
  function updateChoice(selection: string) {
    setSearchQuery("");
...
export type { SearchProvider, AgentWebSearchSelectionProps };
