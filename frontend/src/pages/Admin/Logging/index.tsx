...
export default function AdminLogs(): JSX.Element {
  ...
  const { result, isLoading, error } = useEventLogs(offset) as {
    result: LogResult;
    isLoading: boolean;
    error: Error | undefined;
  };
...
function LogsContainer({
  ...
  handleNext,
  handlePrevious,
}: LogsContainerProps): JSX.Element {
  const { t } = useTranslation();
  if (loading) {
    return (
      <Skeleton
...
// Type-only helper for the layout children
function LayoutWrapper({ children }: { children: ReactNode }): JSX.Element {
  return <>{children}</>;
}
