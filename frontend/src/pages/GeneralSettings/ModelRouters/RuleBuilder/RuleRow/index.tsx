...
const COMPARATOR_SYMBOLS: Record<ComparatorKey, string> = {
  gt: ">",
  gte: ">=",
  lt: "<",
  lte: "<=",
...
function createComparatorLabels(t: TFunction) {
  return {
...
function getComparatorLabel(t: TFunction, comparator: string): string {
  const labels = createComparatorLabels(t) as Record<string, string>;
...
function LLMRuleBody({ rule }: { rule: Rule }): JSX.Element {
  return (
...
function CalculatedRuleBody({ rule }: { rule: Rule }): JSX.Element {
  const { t } = useTranslation();
