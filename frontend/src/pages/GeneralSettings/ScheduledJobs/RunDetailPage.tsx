...
interface RunResult {
  ...
  thoughts?: string[];
  toolCalls?: any[];
  outputs?: { type: string; payload: any }[];
  text?: string;
  metrics?: {
    model?: string;
      ...
      <div className="flex items-center gap-2">
        ...
        {run.status === "completed" && (
          <button
            ...
            disabled={continuing}
            className="border-none h-9 px-5 rounded-lg bg-zinc-50 text-zinc-950 light:bg-slate-900 light:text-theme-text-primary text-sm font-medium hover:bg-zinc-200 light:hover:bg-slate-800 transition-colors disabled:opacity-50 shrink-0"
          >
            {continuing
...
function ErrorSection({ t, error }: { t: any; error?: string }) {
...
function AgentThoughtsSection({ t, result }: { t: any; result: RunResult }) {
...
function ToolCallsSection({ t, result }: { t: any; result: RunResult }) {
...
function GeneratedFilesSection({ t, result }: { t: any; result: RunResult }) {
...
function FinalResponseSection({ t, result }: { t: any; result: RunResult }) {
  if (!result.text) return null;
  let reasoning: string | null = null;
  let msgToRender = result.text;

  if (result.text.match(THOUGHT_REGEX_COMPLETE)) {
    reasoning = result.text.match(THOUGHT_REGEX_COMPLETE)?.[0] || null;
    if (reasoning)
      reasoning = reasoning
...
function MetricsSection({
  ...
  function renderModel(metrics: NonNullable<RunResult["metrics"]>) {
