...
type NewIconFormProps = {
  icon?: IconName;
  url?: string;
  onSave: (icon: IconName, url: string) => void;
  onRemove: () => void;
};

export default function NewIconForm({
  icon,
  url,
  onSave,
  onRemove,
}: NewIconFormProps): JSX.Element {
  const { t } = useTranslation();
  const [selectedIcon, setSelectedIcon] = useState<IconName>(icon || "Plus");
  ...
  const IconComponent = ICON_COMPONENTS[selectedIcon] || Plus;
      ...
      {selectedIcon !== "Plus" && (
        <>
          {isEdited ? (
            ...
            >
              {t("footerCustomization.newIconForm.save")}
            </button>
          ) : (
            <button
              type="button"
