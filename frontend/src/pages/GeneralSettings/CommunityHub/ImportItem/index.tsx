...
export default function CommunityHubImportItemFlow(): JSX.Element {
  const { t } = useTranslation();
  const [step, setStep] = useState("itemId");

  const StepPage = CommunityHubImportItemSteps.hasOwnProperty(step)
    ? (CommunityHubImportItemSteps as Record<string, StepDef>)[step]
    : (CommunityHubImportItemSteps as Record<string, StepDef>).itemId;

  return (
    <CommunityHubImportItemLayout setStep={setStep}>
      {(settings: any, setSettings: any, setStep: any) => (
