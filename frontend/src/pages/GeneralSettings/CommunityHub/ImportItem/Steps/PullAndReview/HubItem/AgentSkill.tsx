...
function FileReview({ item }: FileReviewProps): JSX.Element | null {
  const { t } = useTranslation();
  const files = item.manifest?.files || [];
  const [index, setIndex] = useState(0);
  const [file, setFile] = useState<FileItem>(files[index]);
  ...
  function fileMarkup(file: FileItem): string {
