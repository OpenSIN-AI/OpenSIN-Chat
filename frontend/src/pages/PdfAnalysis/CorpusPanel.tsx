...
// Purpose: Corpus comparison panel: upload PDFs, run cross-document analysis, and view reports.
...
interface CorpusConflictPosition {
  document: string;
  pages?: number[];
  claim: string;
}

interface CorpusConflict {
  topic: string;
  positions?: CorpusConflictPosition[];
}
...
function CorpusForm({ onStarted }: CorpusFormProps) {
  const { t } = useTranslation();
...
interface CorpusReportModalProps {
  job: CorpusJob;
  onClose: () => void;
}

function CorpusReportModal({ job, onClose }: CorpusReportModalProps) {
  const { t } = useTranslation();
