...
export default function FilesystemSidebar() {
  ...
  const rows = data
    ? [
        {
          icon: Cpu,
          label: t("sidebar.filesystem.platform", "Plattform"),
          value: `${data.platform} (${data.arch})`,
        },
        {
          icon: Cpu,
          label: t("sidebar.filesystem.node", "Node.js"),
          value: data.nodeVersion,
        },
        {
          icon: HardDrive,
          label: t("sidebar.filesystem.storage", "Speicher frei"),
          value:
            data.storage?.current != null
              ? `${data.storage.current} GB / ${data.storage.capacity} GB`
              : "—",
        },
        {
          icon: Cpu,
          label: t("sidebar.filesystem.memory", "RAM frei"),
          value: `${data.freeMemMB} MB / ${data.totalMemMB} MB`,
        },
        {
          icon: FolderOpen,
          label: t("sidebar.filesystem.uploadPath", "Storage"),
          value: data.uploadPath,
        },
        {
          icon: Folder,
          label: t("sidebar.filesystem.workDir", "Arbeitsverzeichnis"),
          value: data.workDir,
        },
        {
          icon: HardDrive,
          label: t("sidebar.filesystem.uptime", "Uptime"),
        ...
        <div className="flex-1 overflow-y-auto p-4 no-scroll">
              ...
              {rows.map(({ icon: Icon, label, value }) => (
                <div key={label} className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs font-mono text-zinc-200 light:text-slate-800 mt-0.5 break-all">
                      {value}
                    </p>
                  </div>
                </div>
          ...
          <div className="mt-6 p-3 rounded-lg bg-zinc-800 light:bg-slate-100 border border-zinc-700 light:border-slate-200">
            ...
            <p className="text-xs text-zinc-400 light:text-slate-500 leading-relaxed">
              {t(
                "sidebar.filesystem.hint",
                "Detaillierte Verzeichnisinhalte sind über die Konsole (Terminal-Tab) zugänglich.",
              )}
            </p>
          </div>
        </div>
