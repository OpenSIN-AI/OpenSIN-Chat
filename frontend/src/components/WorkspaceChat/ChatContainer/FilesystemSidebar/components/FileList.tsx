...
import { getFileIcon, getFileColor, formatSize, formatRelativeDate } from "../helpers";
  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      {folders.map((item) => {
        ...
        const subFolders = folderItems.filter((i: any) => i.type === "directory");
        ...
        return (
          <div key={item.path}>
            ...
            >
              {isExpanded ? (
                <CaretDown size={12} weight="bold" className="text-zinc-500 light:text-slate-400 flex-shrink-0" />
              ) : (
                <CaretRight size={12} weight="bold" className="text-zinc-500 light:text-slate-400 flex-shrink-0" />
              )}
              <div className="w-7 h-7 rounded-lg bg-white/5 light:bg-slate-100 flex items-center justify-center flex-shrink-0">
                <Folder
                  size={14}
                  weight="fill"
                  className={isExpanded ? "text-zinc-200 light:text-slate-600" : "text-zinc-400 light:text-slate-500"}
                />
              </div>
              ...
              <div className="ml-4 border-l border-white/5 light:border-slate-200 pl-1">
                {folderLoading && (
                  <div className="px-2.5 py-2">
                    <SpinnerGap size={12} weight="bold" className="animate-spin text-zinc-500" />
                  </div>
                )}
                {folderState?.error && (
                ...
                {!folderLoading && !folderState?.error && subFolders.map((sub: any) => (
                  <div
                    key={sub.path}
                  ...
                  >
                    {expandedFolders[sub.path] ? (
                      <CaretDown size={10} weight="bold" className="text-zinc-500 flex-shrink-0" />
                    ) : (
                      <CaretRight size={10} weight="bold" className="text-zinc-500 flex-shrink-0" />
                    )}
                    <Folder size={12} weight="fill" className="text-zinc-400 light:text-slate-500 flex-shrink-0" />
                    <p className="text-xs text-theme-text-primary light:text-theme-text-primary font-medium truncate flex-1 min-w-0">
                      {sub.name}
                    </p>
                  </div>
                ))}
                {!folderLoading && !folderState?.error && subFiles.map((file: any) => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-6 h-6 rounded-md bg-white/5 light:bg-slate-100 flex items-center justify-center flex-shrink-0">
                        <Icon size={12} weight="regular" className={iconColor} />
                      </div>
                      <div className="flex-1 min-w-0">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {item.modifiedAt && (
                      <>
                        <span className="text-zinc-700 light:text-slate-300 text-xs">·</span>
                        <span className="text-xs text-zinc-500 light:text-slate-400">
                          {formatRelativeDate(item.modifiedAt, t)}
                        </span>
                      </>
                ...
                {isSelected && (
                  <CheckCircle size={16} weight="fill" className="text-zinc-300 light:text-slate-500 flex-shrink-0" />
                )}
                <div className="flex items-center gap-0.5 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    type="button"
