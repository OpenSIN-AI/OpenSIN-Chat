...
function ToolButton({ active = false, label, onClick, children, disabled = false }: any) {
  return (
    <button
      type="button"
      aria-label={label}
...
export default function NoteEditor({ content, editable = true, placeholder, onChange }: NoteEditorProps) {
  ...
  const editor = useEditor({
    extensions: [
      StarterKit,
      Underline,
      Link.configure({ openOnClick: !editable, autolink: true, defaultProtocol: "https" }),
  ...
  const insertMedia = useCallback((kind: "image" | "youtube") => {
    if (!editor) return;
    const url = window.prompt(kind === "youtube" ? "YouTube-URL" : "Bild-URL");
    if (!url) return;
    if (kind === "youtube") editor.chain().focus().setYoutubeVideo({ src: url }).run();
  ...
  if (!editor) return <div className="flex-1 animate-pulse bg-theme-bg-secondary" />;

  return (
    <div className="flex min-h-0 flex-1 flex-col bg-theme-bg-primary">
      {editable && (
        <div className="relative flex shrink-0 items-center gap-0.5 overflow-x-auto border-b border-theme-modal-border px-2 py-1.5" role="toolbar" aria-label="Text formatieren">
          <ToolButton label="Rückgängig" disabled={!editor.can().undo()} onClick={() => editor.chain().focus().undo().run()}><ArrowCounterClockwise size={15} /></ToolButton>
          <ToolButton label="Wiederholen" disabled={!editor.can().redo()} onClick={() => editor.chain().focus().redo().run()}><ArrowClockwise size={15} /></ToolButton>
          <span className="mx-1 h-4 w-px shrink-0 bg-theme-modal-border" />
          <ToolButton label="Überschrift 1" active={editor.isActive("heading", { level: 1 })} onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()}><TextHOne size={15} /></ToolButton>
          <ToolButton label="Überschrift 2" active={editor.isActive("heading", { level: 2 })} onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()}><TextHTwo size={15} /></ToolButton>
          <ToolButton label="Fett" active={editor.isActive("bold")} onClick={() => editor.chain().focus().toggleBold().run()}><TextB size={15} /></ToolButton>
          <ToolButton label="Kursiv" active={editor.isActive("italic")} onClick={() => editor.chain().focus().toggleItalic().run()}><TextItalic size={15} /></ToolButton>
          <ToolButton label="Unterstrichen" active={editor.isActive("underline")} onClick={() => editor.chain().focus().toggleUnderline().run()}><TextUnderline size={15} /></ToolButton>
          <ToolButton label="Durchgestrichen" active={editor.isActive("strike")} onClick={() => editor.chain().focus().toggleStrike().run()}><TextStrikethrough size={15} /></ToolButton>
          <ToolButton label="Aufzählung" active={editor.isActive("bulletList")} onClick={() => editor.chain().focus().toggleBulletList().run()}><ListBullets size={15} /></ToolButton>
          <ToolButton label="Nummerierte Liste" active={editor.isActive("orderedList")} onClick={() => editor.chain().focus().toggleOrderedList().run()}><ListNumbers size={15} /></ToolButton>
          <ToolButton label="Todo" active={editor.isActive("taskList")} onClick={() => editor.chain().focus().toggleTaskList().run()}><CheckSquare size={15} /></ToolButton>
          <ToolButton label="Zitat" active={editor.isActive("blockquote")} onClick={() => editor.chain().focus().toggleBlockquote().run()}><Quotes size={15} /></ToolButton>
          <ToolButton label="Codeblock" active={editor.isActive("codeBlock")} onClick={() => editor.chain().focus().toggleCodeBlock().run()}><Code size={15} /></ToolButton>
          <ToolButton label="Link" active={editor.isActive("link")} onClick={insertLink}><LinkIcon size={15} /></ToolButton>
          <div className="relative">
            <ToolButton label="Inhalt einfügen" active={insertOpen} onClick={() => setInsertOpen((value) => !value)}>+</ToolButton>
            {insertOpen && (
