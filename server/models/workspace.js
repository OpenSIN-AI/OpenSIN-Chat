...
const Workspace = {
  ...
  update: async function (id = null, updates = {}) {
    ...
    // When switching to opensin-router, chatModel is not used.
    // When switching away from opensin-router, clear router_id.
    if (validatedUpdates?.chatProvider === "opensin-router") {
      validatedUpdates.chatModel = null;
    } else if (
      validatedUpdates?.chatProvider &&
      validatedUpdates.chatProvider !== "opensin-router"
    ) {
      validatedUpdates.router_id = null;
    }
