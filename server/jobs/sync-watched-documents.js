  ...
  try {
    ...
    for (const queue of queuesToProcess) {
      try {
        ...
        // A single queue failure must not abort remaining stale documents.
        console.error(
          `[sync-watched-documents] Queue ${queue.id} failed:`,
          queueErr.message,
        );
        log(
          `Queue ${queue.id} errored: ${queueErr.message}. Continuing with next queue.`,
        );
        try {
          await DocumentSyncQueue.saveRun(
            queue.id,
            DocumentSyncRun.statuses.failed,
            {
              filename: queue.workspaceDoc?.filename || "unknown",
              workspacesModified: [],
