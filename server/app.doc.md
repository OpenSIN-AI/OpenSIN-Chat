...
In production, the fallback `app.use("/", ...)` handler checks whether the
request path is `/docs` or `/docs/:slug`. If a matching pre-rendered HTML
fragment exists in `server/public/docs/<slug>.html` (generated by
`frontend/scripts/prerender-docs.mjs` at build time), the handler passes it to
`MetaGenerator.generate()` so the initial docs paint is rendered HTML rather than
an empty `<div id="root">`.

If no fragment is found (e.g. the slug does not exist), the request falls back
to the normal SPA shell and the React router handles the 404 page.
