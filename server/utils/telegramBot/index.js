...
module.exports = { TelegramBotService };BOT_COMMANDS.find(
        (c) => c.command === "history",
      );
      if (!cmd) return;
      const handler = cmd.initHandler();
      guard(msg, () => handler(ctx, msg.chat.id, msg.text));m/yagop/node-telegram-bot-api/blob/master/doc/usage.md#sending-files
process.env.NTBA_FIX_350 = 1;
const TelegramBot = require("node-telegram-bot-api");
const {
  ExternalCommunicationConnector,
