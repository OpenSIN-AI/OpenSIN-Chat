  ...
  static saveFlow(name, config, uuid = null) {
    try {
      ...
      if (!supportsAllBlocks)
        throw new Error(
          "This flow includes unsupported blocks. They may not be supported by your version of OpenSIN Chat or are not available on this platform.",
        );
          ...
          aibitat.function({
            ...
            handler: async (args) => {
              ...
              if (!result.success) {
                ...
                aibitat.introspect(
                  `Flow failed: ${failedStep?.error || "Unknown error"}`,
                );
                return `Flow execution failed: ${failedStep?.error || "Unknown error"}`;
              }
