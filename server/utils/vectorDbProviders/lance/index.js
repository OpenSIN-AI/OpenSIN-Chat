...
class LanceDb extends VectorDatabase {
  ...
  async deleteDocumentFromNamespace(namespace, docId) {
    ...
    // Escape single quotes in vector IDs to prevent SQL injection in the
    // LanceDB filter expression. Vector IDs are UUIDs generated by the
    // system, but defense-in-depth requires escaping anyway.
    const escapedIds = vectorIds
      .map((v) => `'${String(v).replace(/'/g, "''")}'`)
      .join(",");
    await table.delete(`id IN (${escapedIds})`);

    // Clean up DocumentVectors records — all other providers do this
    // (Chroma, Qdrant, Milvus, Pinecone, Weaviate). Without it, stale
    // entries remain in the database pointing to deleted vectors.
