...
class PGVector extends VectorDatabase {
   ...
   * - Defaults to "openafd_vectors" if no table name is provided.
   * @returns {string}
   */
  static tableName() {
    return process.env.PGVECTOR_TABLE_NAME || "openafd_vectors";
  }
   ...
   * so that openafd can manage the table directly.
   *
  ...
  /**
   * Check if the openafd embedding table exists in the database
   * @returns {Promise<boolean>}
   */
  async dbTableExists() {
    let connection = null;
  ...
  /**
   * Reset the entire vector database table associated with openafd
