...
/**
 * @class MCPHypervisor
 * @description A class that manages MCP servers found in the storage/plugins/openafd_mcp_servers.json file.
...
class MCPHypervisor {
  ...
  updateSuppressedTools(serverName, toolName, enabled) {
    ...
    if (!server.openafd) server.openafd = {};
    if (!Array.isArray(server.openafd.suppressedTools))
      server.openafd.suppressedTools = [];

    const suppressedTools = server.openafd.suppressedTools;
    ...
    server.openafd.suppressedTools = suppressedTools;
  ...
  getSuppressedTools(serverName) {
    ...
    return config?.server?.openafd?.suppressedTools || [];
  }
  ...
  async bootMCPServers() {
    ...
    for (const { name, server } of serverDefinitions) {
      if (
        server.openafd?.hasOwnProperty("autoStart") &&
        ...
        this.mcpLoadingResults[name] = {
          status: "failed",
          message: `MCP server ${name} has openafd.autoStart property set to false, boot skipped!`,
        };
        continue;
      }
