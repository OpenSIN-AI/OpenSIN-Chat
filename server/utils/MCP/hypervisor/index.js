...
/**
 * @class MCPHypervisor
 * @description A class that manages MCP servers found in the storage/plugins/opensin_mcp_servers.json file.
 ...
 * For example, if the tools requires `npx` then the context in which OpenSIN Chat mains process is running will need to access npx.
 * This is typically not common in our pre-built image so may not function. But this is the case anywhere MCP is used.
 *
 * OpenSIN Chat will take care of porting MCP servers to agent-callable functions via @agent directive.
...
class MCPHypervisor {
  ...
  updateSuppressedTools(serverName, toolName, enabled) {
    ...
    if (!server.opensin) server.opensin = {};
    if (!Array.isArray(server.opensin.suppressedTools))
      server.opensin.suppressedTools = [];

    const suppressedTools = server.opensin.suppressedTools;
    ...
    server.opensin.suppressedTools = suppressedTools;
  ...
  getSuppressedTools(serverName) {
    ...
    return config?.server?.opensin?.suppressedTools || [];
  }
  ...
  async bootMCPServers() {
    ...
    for (const { name, server } of serverDefinitions) {
      if (
        server.opensin?.hasOwnProperty("autoStart") &&
        server.opensin.autoStart === false
      ) {
        this.log(
          `MCP server ${name} has opensin.autoStart property set to false, skipping boot!`,
        );
        this.mcpLoadingResults[name] = {
          status: "failed",
          message: `MCP server ${name} has opensin.autoStart property set to false, boot skipped!`,
        };
        continue;
      }
