...
const { parseChatRequestContext } = require("./requestContext");
...
async function streamChatHandler(request, response, { thread = null } = {}) {
  ...
  try {
    ...
    // Centralized request context validation — turnId, notebookMode, selectedSourceIds,
    // sourceSelectionExplicit, codeRunnerId all validated and normalized here.
    const requestContext = parseChatRequestContext(body);
