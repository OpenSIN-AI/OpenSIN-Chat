...
- Resolves the active LLM connector for a workspace via `resolveProviderConnector()`,
  transparently handling both direct providers and the `openafd-router` model router.
...
- `process.env.MODEL_ROUTER_ID` — default model router when `chatProvider` is `openafd-router`.
