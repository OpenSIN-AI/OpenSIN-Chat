...
class TriggerEngine {
   ...
   * Delegates to triggerPollers for per-connector implementations:
   *   - gmail: uses GmailBridge to search recent messages
   *   - github: fetches issues/PRs since last checkpoint
   *   - rss/atom: parses feed and compares item GUIDs
   *   - webhook: no-op (push-based)
   */
  async _pollConnector(config, checkpoint) {
    const { pollConnector } = require("./triggerPollers");
    return await pollConnector(config, checkpoint);
  }
  /**
