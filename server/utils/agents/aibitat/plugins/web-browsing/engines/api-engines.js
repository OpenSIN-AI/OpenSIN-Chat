    ...
    switch (engine) {
      case "google":
        ...
        response.local_results?.forEach((searchResult) => {
          const { title, rating, reviews, description, address, website, extensions } = searchResult;
          data.push({ title, rating, reviews, description, address, website, extensions });
        });
        break;
      case "google_maps":
      ...
      case "google_shopping_light":
        response.shopping_results?.slice(0, 10).forEach((searchResult) => {
          const { title, source, price, rating, reviews, snippet, thumbnail, product_link } = searchResult;
          data.push({ title, source, price, rating, reviews, snippet, thumbnail, product_link });
        });
        break;
      case "google_news_light":
        response.news_results?.slice(0, 10).forEach((searchResult) => {
          const { title, link, source, thumbnail, snippet, date } = searchResult;
          data.push({ title, link, source, thumbnail, snippet, date });
        });
        break;
      case "google_jobs":
        response.jobs_results?.forEach((searchResult) => {
          const { title, company_name, location, description, apply_options, extensions } = searchResult;
          data.push({ title, company_name, location, description, apply_options, extensions });
        });
        break;
      case "google_patents":
        response.organic_results?.forEach((searchResult) => {
          const { title, patent_link, snippet, inventor, assignee, publication_number } = searchResult;
          data.push({ title, patent_link, snippet, inventor, assignee, publication_number });
        });
        break;
      case "google_scholar":
        response.organic_results?.forEach((searchResult) => {
      ...
      case "amazon":
        response.organic_results?.slice(0, 10).forEach((searchResult) => {
          const { title, rating, reviews, price, link_clean, thumbnail } = searchResult;
