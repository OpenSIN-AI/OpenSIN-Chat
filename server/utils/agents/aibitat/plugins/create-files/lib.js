...
class CreateFilesManager {
  ...
  getLogo({ forDarkBackground = false, format = "buffer" } = {}) {
    ...
    // forDarkBackground=true  → white logo (opensin-logo-dark.png) for dark backgrounds
    // forDarkBackground=false → dark logo  (opensin-logo.png)       for light backgrounds
    const filename = forDarkBackground
      ? "opensin-logo-dark.png"
      : "opensin-logo.png";
    try {
      if (format === "dataUri") {
